...
echo "socat listens on 'aws-signing-helper:$PORT' and forwards requests to 'localhost:$PORT', where aws-signing-helper is listening"
echo "export AWS_EC2_METADATA_SERVICE_ENDPOINT=http://aws-signing-helper:$PORT/ in docker containers to make use of credentials provided via aws-signing-helper service"
